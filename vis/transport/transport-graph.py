...
df = pd.read_csv(
    "C:\\Users\\ismae\\Documents\\GitHub\\dataseeds.github.io\\vis\\transport\\change-energy-consumption.csv")

df.rename(columns={"geo": "Eurozone", "share": "Share"}, inplace=True)
df["Transport mode"] = df["Transport mode"].str.replace(
    "Final consumption - transport sector - ", "").str.title()
...
fig.add_trace(
    go.Choropleth(colorscale='BuGn', hovertext=[],
...
button_EEA33 = dict(method='update', label='EEA-33',
                    args=[{"locations": [EEA33],
                           "cells":  dict(
                        values=[dff[k].tolist() for k in dff.columns],
                        fill=dict(color=[["#86CDB9", "#e5ecf6", "#e5ecf6"]]))}])
button_EU13 = dict(method='update', label='EU-13',
                   args=[{"locations": [EU13],
                          "cells":  dict(
                       values=[dff[k].tolist() for k in dff.columns],
                       fill=dict(color=[["#e5ecf6", "#86CDB9", "#e5ecf6"]]))}])
button_EU15 = dict(method='update', label='EU-15',
                   args=[{"locations": [EU15],
                          "cells":  dict(
                       values=[dff[k].tolist() for k in dff.columns],
                       fill=dict(color=[["#e5ecf6", "#e5ecf6", "#86CDB9"]]))}])
...
fig.write_html("C:\\Users\\ismae\\Documents\\GitHub\\dataseeds.github.io\\pages\\transport-graph.html",
               config=conf)
